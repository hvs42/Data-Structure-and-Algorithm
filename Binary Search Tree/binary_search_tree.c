...
NODE* createtree(NODE *node, int data)          //Creating BST or Insertion in BST
{
    if (node == NULL)
    {
        NODE *temp;
...
NODE* search(NODE *node, int data)      //Searching in BST
{
    if(node == NULL)
    {
        printf("\nElement not found");
...
NODE* findMin(NODE *node)       //Finding Minimum element of BST
{
    if(node==NULL)
    {
        return NULL;
...
NODE* del(NODE *node, int data)         //deletion of BST
{
    NODE *temp;
    if(node == NULL)
    {
